...
public extension UIColor {
    ...
    // MARK: self to struct
     func hsla(alpha: CGFloat = 1.0) -> HSLA {
        //return self.hsla()
        let rgba = self.rgba()
    ...
    // MARK: constructors
     ...
     convenience init(hsla: HSLA, alpha: CGFloat = 1.0) {
        let rgba = UIColor.hslaToRGBA(hsla: hsla)
        self.init(red: rgba.red, green: rgba.green, blue: rgba.blue, alpha: rgba.alpha)
    }
    
    
     class func colorWith(hsla: HSLA) -> UIColor {
        return hsla.color()
    }
    
    /* Name collision with UIColor */
     class func colorWith(hue: CGFloat, saturation: CGFloat, lightness: CGFloat, alpha: CGFloat) -> UIColor {
    ...
    // http://www.easyrgb.com/index.php?X=MATH
     class func hslaToRGBA(hsla: HSLA) -> RGBA {
     ...
     class func hslaToCMYKA(hsla: HSLA) -> CMYKA {
        let color = UIColor.hslaToRGBA(hsla: hsla)
        let cmyka = color.cmyka()
        return cmyka
    }
    
     class func hslaToYUVA(hsla: HSLA) -> YUVA {
