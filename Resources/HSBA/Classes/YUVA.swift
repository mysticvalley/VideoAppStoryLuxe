...
public extension UIColor {
    ...
    // MARK: self to struct
     ...
     func yuva(alpha: CGFloat = 1.0) -> YUVA {
        let rgba = self.rgba()
    ...
    // MARK: constructors
     ...
     convenience init(yuva: YUVA, alpha: CGFloat = 1.0) {
        let rgba = UIColor.yuvaToRGBA(yuva: yuva)
        self.init(red: rgba.red, green: rgba.green, blue: rgba.blue, alpha: rgba.alpha)
    }
    
    
     class func colorWith(yuva: YUVA) -> UIColor {
        return yuva.color()
    }
    
     class func colorWith(y: CGFloat, u: CGFloat, v: CGFloat, alpha: CGFloat) -> UIColor {
    ...
    class func yuvaToRGBA(yuva: YUVA) -> RGBA {
...
//        }
     ...
     class func yuvaToHSBA(yuva: YUVA) -> HSBA {
        let rgba = UIColor.yuvaToRGBA(yuva: yuva)
        let hsba = UIColor.rgbaToHSBA(rgba: rgba)
        return hsba
    }
    
     class func yuvaToHSLA(yuva: YUVA) -> HSLA {
        let rgba = UIColor.yuvaToRGBA(yuva: yuva)
        let hsla = UIColor.rgbaToHSLA(rgba: rgba)
        return hsla
    }
     ...
     class func yuvaToCMYKA(yuva: YUVA) -> CMYKA {
        let rgba = UIColor.yuvaToRGBA(yuva: yuva)
        let cmyka = UIColor.rgbaToCMYKA(rgba: rgba)
        return cmyka
    }
