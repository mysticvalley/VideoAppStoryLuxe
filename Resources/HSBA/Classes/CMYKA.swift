...
public extension UIColor {
    ...
    // MARK: constructors
     ...
     convenience init(cmyka: CMYKA, alpha: CGFloat = 1.0) {
        let rgba = UIColor.cmykaToRGBA(cmyka: cmyka)
        self.init(red: rgba.red, green: rgba.green, blue: rgba.blue, alpha: rgba.alpha)
    }
    
     class func colorWith(cmyka: CMYKA) -> UIColor {
        return cmyka.color()
    }
    
    
     class func colorWith(cyan: CGFloat, magenta: CGFloat, yellow: CGFloat, black: CGFloat, alpha: CGFloat) -> UIColor {
    ...
    // http://www.rapidtables.com/convert/color/cmyk-to-rgb.htm
     class func cmykaToRGBA(cmyka: CMYKA) -> RGBA {
     ...
     class func cmykaToHSBA(cmyka: CMYKA) -> HSBA {
        let rgba = UIColor.cmykaToRGBA(cmyka: cmyka)
     ...
     class func cmykaToHSLA(cmyka: CMYKA) -> HSLA {
        let rgba = UIColor.cmykaToRGBA(cmyka: cmyka)
        let hsla = UIColor.rgbaToHSLA(rgba: rgba)
        return hsla
    }
    
     class func cmykaToYUVA(cmyka: CMYKA) -> YUVA {
        let rgba = UIColor.cmykaToRGBA(cmyka: cmyka)
